...
#if defined(__CUDACC__) || __HCC__ == 1 || __HIP__ == 1 // __CUDACC__ 用于区分单间代码是否由NVCC编译，还是其他编译器
// Designates functions callable from the host (CPU) and the device (GPU) // 
#define HOST_DEVICE __host__ __device__
#define HOST_DEVICE_INLINE HOST_DEVICE __forceinline__
#else
#include <algorithm>
#define HOST_DEVICE // TODO: 这里宏定义了一个空
#define HOST_DEVICE_INLINE HOST_DEVICE inline // 把HOST_DEVICE_INLINE HOST_DEVICE 都声明为inline类型
#endif
...
struct Point {
  T x, y;
  HOST_DEVICE_INLINE Point(const T& px = 0, const T& py = 0) : x(px), y(py) {} // 列表初始化
...
// 使用T进行模板嵌套，其中<T>的T代表所要使用的类型与本级的相同
template <typename T>
